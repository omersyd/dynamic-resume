...
Your goal is to create a "Content Strategy" for a resume that bridges a candidate's experience with a target job.
...
2. EXPERIENCE SELECTION: Which specific roles or projects from the candidate's history are most relevant?
   Which ones should be minimized?
3. BULLET POINT ANGLES: For the top relevant roles, explain how to frame the achievements to match the Job Analysis.
   - Example: (
...
4. KEYWORD INTEGRATION: List specific keywords from the analysis that must be naturally woven into the bullet points.
